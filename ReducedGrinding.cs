    ...
    class ReducedGrinding : Mod
    {
        ...
        public class VanillaBagsAndExtractinator : GlobalItem  //Rates show in comments are in addition to vanilla rates.
        {
            public override void OpenVanillaBag(string context, Player player, int arg)
            {
                ...
                {
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBoneRattleIncrease, ItemID.BoneRattle);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBossMaskIncrease, ItemID.BrainMask);
                }
                else if (arg == ItemID.FishronBossBag)
                {
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootFishronWingsIncrease, ItemID.FishronWings);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootFishronTruffleworm, ItemID.TruffleWorm);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBossMaskIncrease, ItemID.DukeFishronMask);
                }
                else if (arg == ItemID.EaterOfWorldsBossBag)
                {
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootEatersBoneIncrease, ItemID.EatersBone);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBossMaskIncrease, ItemID.EaterMask);
                }
                else if (arg == ItemID.EyeOfCthulhuBossBag)
                {
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBinocularsIncrease, ItemID.Binoculars);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBossMaskIncrease, ItemID.EyeMask);
                }
                else if (arg == ItemID.PlanteraBossBag)
                {
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootTheAxeIncrease, ItemID.TheAxe);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootSeedlingIncrease, ItemID.Seedling);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBossMaskIncrease, ItemID.PlanteraMask);
                }
                else if (arg == ItemID.QueenBeeBossBag)
                {
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootHoneyedGogglesIncrease, ItemID.HoneyedGoggles);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootNectarIncrease, ItemID.Nectar);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBossMaskIncrease, ItemID.BeeMask);
                }
                else if (arg == ItemID.MoonLordBossBag)
                {
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBossMaskIncrease, ItemID.BossMaskMoonlord);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootMoonLordEachWeaponIncrease, ItemID.Meowmere);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootMoonLordEachWeaponIncrease, ItemID.Terrarian);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootMoonLordEachWeaponIncrease, ItemID.StarWrath);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootMoonLordEachWeaponIncrease, ItemID.SDMG);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootMoonLordEachWeaponIncrease, ItemID.FireworksLauncher); //Celebration
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootMoonLordEachWeaponIncrease, ItemID.LastPrism);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootMoonLordEachWeaponIncrease, ItemID.LunarFlareBook);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootMoonLordEachWeaponIncrease, ItemID.RainbowCrystalStaff);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootMoonLordEachWeaponIncrease, ItemID.MoonlordTurretStaff); //Lunar Portal Staff
                }
                else if (arg == ItemID.SkeletronBossBag)
                {
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBookofSkullsIncrease, ItemID.BookofSkulls);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootSkeletronBoneKey, ItemID.BoneKey);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBossMaskIncrease, ItemID.SkeletronMask);
                }
                else if (arg == 3318) //King Slime
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBossMaskIncrease, ItemID.KingSlimeMask);
                else if (arg == 3324) //Wall of Flesh
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBossMaskIncrease, ItemID.FleshMask);
                else if (arg == 3325) //The Destroyer
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBossMaskIncrease, ItemID.DestroyerMask);
                else if (arg == 3326) //The Twins
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBossMaskIncrease, ItemID.TwinMask);
                else if (arg == 3327) //Skeletron Prime
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBossMaskIncrease, ItemID.SkeletronPrimeMask);
                else if (arg == 3329) //Golem
                {
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBossMaskIncrease, ItemID.GolemMask);
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootPicksawIncrease, ItemID.Picksaw);
                }
                else if (arg == 3860) //Betsy
                    grab_bag_drop(GetInstance<AEnemyDropConfig>().LootBossMaskIncrease, ItemID.BossMaskBetsy);
