	...
	class ReducedGrinding : Mod
    {
        ...
        public class BossBags : GlobalItem  //Rates show in comments are in addition to vanilla rates.
		{
			public override void OpenVanillaBag(string context, Player player, int arg)
			{
				...
				{
					if (Main.rand.NextFloat() < Config.LootMoonLordEachWeaponIncrease)
					{
				...
				else if (arg == 3205) //Dungeon Crate
				{
					...
					if (Main.rand.NextFloat() < Config.CrateDungeonHardmodeGoldenLockBoxIncrease && Main.hardMode)
					{
		...
		public class ModGlobalNPC : GlobalNPC
		{
			public override void NPCLoot(NPC npc)
			{
				if (npc.lifeMax > 5 && npc.type != NPCID.Bee && npc.type != NPCID.BeeSmall)
				{
					...
					for (int j = 0; j < Config.DropTriesForAllEnemyDroppedLoot; j++)
					{
						...
						if (npc.type == NPCID.MoonLordCore)
						{
							...
							if (!Main.expertMode)
							{
								...
								if (Main.rand.NextFloat() < Config.LootMoonLordEachWeaponIncrease * difficultyMultiplier)
								{
						...
						if (npc.type == NPCID.MartianSaucerCore)
						{
							...
							if (Main.rand.NextFloat() < Config.MartianSaucerMartianCallDrop)
						...
						{
							if (Main.rand.NextFloat() < Config.LootDesertFossilFromDuneSplicer)
							{
								Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemID.DesertFossil, 1, false, -1, false, false);
							}
							sandmin = Config.LootMinSandFromDuneSplicer;
							sandmax = Config.LootMaxSandFromDuneSplicer;
						...
						{
							sandmin = Config.LootMinSandFromTombCrawler;
							sandmax = Config.LootMaxSandFromTombCrawler;
							if (sandmin > 0 && sandmax > 0 && sandmax > sandmin)
							{
								Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, sandtype, Main.rand.Next(sandmin, sandmax + 1), false, -1, false, false);
							}
						...
						if (npc.type == NPCID.Hornet || (npc.type >= 231 && npc.type <= 235)) //Hornet
						{
							...
							if (Main.rand.NextFloat() < Config.LootHiveBlockFromHornetsAndMossHornetsAfterQueenBeDowned && NPC.downedQueenBee)
							{
						...
						if (npc.type == NPCID.Harpy)
						{
							...
							if (Main.rand.NextFloat() < Config.LootCloudFromHarpies)
							{
						...
						{
							if (Main.rand.NextFloat() < Config.LootGreenCapForNonAndrewGuide)
							{
						...
						{
							if (Main.rand.NextFloat() < Config.LootExoticScimitarIncrease)
							{
						...
						{
							if (Main.rand.NextFloat() < Config.LootAleTosserIncrease)
							{
						...
						{
							if (Main.rand.NextFloat() < Config.LootStylishScissorsIncrease)
							{
						...
						{
							if (Main.rand.NextFloat() < Config.LootPaintballGunIncrease)
							{
						...
						{
							if (Main.rand.NextFloat() < Config.LootHappyGrenadeIncrease)
							{
						...
						if (npc.type == NPCID.RainbowSlime && (Config.LootRainbowBlockDropMinIncrease < Config.LootRainbowBlockDropMaxIncrease)) //RainbowSlime
						{
							Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemID.RainbowBrick, Main.rand.Next(Config.LootRainbowBlockDropMinIncrease - 30, Config.LootRainbowBlockDropMaxIncrease - 60), false, -1, false, false); //Rainbow Block
						}
						...
						if (Config.AllEnemiesLootBiomeMatchingFoundOnlyChestDrop > 0)
						{
							...
							else if (player.ZoneSnow && player.ZoneRockLayerHeight && Main.rand.NextFloat() < Config.AllEnemiesLootBiomeMatchingFoundOnlyChestDrop)
								Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, 681, 1, false, -1, false, false); //Ice Chest
							else if (player.ZoneJungle && player.ZoneRockLayerHeight && Main.rand.NextFloat() < Config.AllEnemiesLootBiomeMatchingFoundOnlyChestDrop)
								Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, 680, 1, false, -1, false, false); //Ivy Chest
							else if ((Main.tile[npcTileX, npcTileY].wall == WallID.LihzahrdBrickUnsafe && NPC.downedPlantBoss) && Main.rand.NextFloat() < Config.AllEnemiesLootBiomeMatchingFoundOnlyChestDrop) //Lihzahrd Temple Enemies
								Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, 1142, 1, false, -1, false, false); //Lihzahrd Chest
							else if (((npc.type >= 163 && npc.type <= 165) || npc.type == 238) && Main.rand.NextFloat() < Config.AllEnemiesLootBiomeMatchingFoundOnlyChestDrop) //Spider Nest Enemies
								Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, 952, 1, false, -1, false, false); //Web Covered Chest
							else if (player.ZoneUnderworldHeight && Main.rand.NextFloat() < Config.AllEnemiesLootBiomeMatchingFoundOnlyChestDrop)
							...
							else if (player.ZoneOverworldHeight && Main.rand.NextFloat() < Config.AllEnemiesLootBiomeMatchingFoundOnlyChestDrop)
								Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, 831, 1, false, -1, false, false); //Living Wood Chest
							else if ((player.ZoneDirtLayerHeight || player.ZoneRockLayerHeight) && Main.rand.NextFloat() < Config.AllEnemiesLootBiomeMatchingFoundOnlyChestDrop)
								Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, 306, 1, false, -1, false, false); //Gold Chest
							else if (player.ZoneSkyHeight && Main.rand.NextFloat() < Config.AllEnemiesLootBiomeMatchingFoundOnlyChestDrop)
						...
						if (Config.LockBoxesGiveNonFurniture && (npc.type == 57 || npc.type == 58 || (npc.type >= 63 && npc.type <= 65) || npc.type == 67 || npc.type == 102 || npc.type == 103 || npc.type == 157 || npc.type == 220 || npc.type == 221 || npc.type == 241 || npc.type == 242 || npc.type == 256 || npc.type == 465)) //Water Enemies (https://terraria.gamepedia.com/Water#Contents)
						{
						...
						else if (player.ZoneDungeon)
						{
							...
							if (Config.LockBoxesGiveNonFurniture && NPC.downedPlantBoss)
							{
						...
						else if (Config.LockBoxesGiveNonFurniture && ((npc.type >= 163 && npc.type <= 165) || npc.type == 238)) //Spider Nest Enemies
						{
					...
					if (travelingMerchantExists && Config.ChanceThatEnemyKillWillResetTravelingMerchant > 0)
					{
