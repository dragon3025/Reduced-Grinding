		...
		public class ExamplePersonProfile : ITownNPCProfile
		{
			...
			public Asset<Texture2D> GetTextureNPCShouldUse(NPC npc)
			{
				if (npc.IsABestiaryIconDummy && !npc.ForcePartyHatOn)
					return Request<Texture2D>("ReducedGrinding/NPCs/StationaryMerchant");

				if (npc.altTexture == 1)
					return Request<Texture2D>("ReducedGrinding/NPCs/StationaryMerchant_alt");

				return Request<Texture2D>("ReducedGrinding/NPCs/StationaryMerchant");
			}

			public int GetHeadTextureIndex(NPC npc) => GetModHeadSlot("ReducedGrinding/NPCs/StationaryMerchant_Head.png");
		}
